...
app.config['SECURITY_PASSWORD_SALT'] = '0b646d18b4a85fc88f5f9e74002247ec'
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if check_password_hash(user.password, password):
                ...
                error_msg = 'Account locked due to too many failed attempts' if check_login_security(user, request) else 'Invalid email or password'
...
def login_history():
    activities = UserActivity.query.filter_by(user_id=session['user_id'])\
        ...
        .limit(50)\
        .all()
    return render_template('login_history.htm', activities=activities)    
...
def predict():
    ...
    try:
        ...
        if binary_class != 'Colonoscopy':
           ...
           return jsonify({
              'success': False,
              'is_colonoscopy': False,  # Add this line
              'error': 'Invalid image type',
              'message': 'The uploaded image is not a colonoscopy image...',
    ...
    try:
        if 'user_id' in session:
            new_result = PredictionResult(
                ...
                confidence=result_data.get('polyp_confidence'),
...
def delete_account():
    try:
        ...
        # Clear session (must be after successful deletion)
        session.clear()
